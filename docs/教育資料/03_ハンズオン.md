...
3 つの方法から好きなものを選びます（最終的に `cursor-office-automation-starter/` フォルダがローカルにあれば OK）。

#### 方法 A: Cursor の Clone repo（推奨・git 不要）
1. Welcome 画面（または `File → New Window`）の `Clone repo` をクリック
2. URL を貼る: `https://github.com/kurosawa-kuro/cursor-office-automation-starter.git`
...
1. https://github.com/kurosawa-kuro/cursor-office-automation-starter を開く → 緑の `Code` → `Download ZIP`
2. zip を右クリック → `すべて展開` → 展開先（例: `C:\dev\cursor-office-automation-starter`）を覚えておく

### ② Cursor で Open Folder
1. `File → Open Folder`（または Welcome 画面の `Open Folder`）
2. 取得した `cursor-office-automation-starter/` フォルダを選択 → `フォルダーの選択`
